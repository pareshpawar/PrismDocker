...
/_/   /_/  /_/____/_/ /_/ /_/    `
	...
	footer := helpStyle.Render("↑/k: Up • ↓/j: Down • r: Refresh • s: Sort • a: All/Running • t: Stats • q: Quit")
	...
	if len(m.filteredContainers) == 0 {
		...
		for i := start; i < end; i++ {
			...
			var row string
			if m.showStats {
				// Stats columns: CPU%, Mem, Net I/O
				s := m.stats[c.ID]
				cpuStr := fmt.Sprintf("%.1f%%", s.CPUPercent)
				memStr := fmt.Sprintf("%s/%s", formatBytesShort(s.MemUsage), formatBytesShort(s.MemLimit))
